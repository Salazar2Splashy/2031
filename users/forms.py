...
def postcode_check(form, field):
    ...
    if not p.match(field.data) and not p2.match(field.data) and not p3.match(field.data):
        raise ValidationError(
            "Postcode must be in the forms XY YXX, XYY YXX or XXY YXX where X is a letter and Y is a number. Letters must be all in caps.")
...
class RegisterForm(FlaskForm):
    ...
    phone = StringField(
        validators=[Regexp(regex="^[0-9]{4}[-][0-9]{3}[-][0-9]{4}$"), DataRequired(message="Field cannot be blank")])
    dob = StringField(
        validators=[Regexp(regex="^[0-9]{2}[/][0-9]{2}[/][0-9]{4}$"), DataRequired(message="Field cannot be blank")])
